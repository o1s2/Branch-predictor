...
BTB::BTB(int cachesize , int assoc )
{
...
int BTB::Read(unsigned long int pcaddress)
{
	...
	pctemp = pc_address >> 2; //eliminates right most 2 bits
	...
	for (int j = 0; j < BTBassoc; ++j)
	{
		...
		if (BTBmatrix[pcindex][j].tagaddress == pctag)
		{
			//cout << "tag found" << endl;
...
void BTB::getprint(void)
{
	...
	for (int i = 0; i < 10; ++i)
	{
		...
		for (int j = 0; j < BTBassoc; ++j)
		{
			cout << "   	" << hex << BTBmatrix[i][j].tagaddress;
		}
		cout << endl;
	}
	for (int l = 10; l < indexcount; ++l)
	{
		cout << "set " << dec << l << "    :	  ";
